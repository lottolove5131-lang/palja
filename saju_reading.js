...
(function (global) {
  ...
  rule('strength_extreme', 900, (A, P, DW, C, F) => {
    ...
    return page({
      ...
      action: UL(tae ? [
        ...
        '<b>이번 주</b> — 지금 혼자 지고 있는 일 하나를 <b>이름을 정해 넘기세요.</b> 도와달라는 말을 구체적으로 하는 연습입니다.',
        '<b>이번 달</b> — 반복하는 일 하나를 <b>절차로 적어</b> 두세요. 이 사주는 기억력이 아니라 시스템으로 버팁니다.',
  ...
  rule('excess_element', 290, (A, P, DW, C, F) => {
    ...
    return page({
      ...
      action: UL([
        `<b>이번 주</b> — ${EL_LIFE[GEN[e]].act} 쪽으로 작은 일 하나를 끝내세요.`,
        `<b>이번 달</b> — 넘치는 쪽(${EL_LIFE[e].area})에 쓰는 시간을 <b>10%만</b> 줄여 출구 쪽으로 옮기세요.`,
  ...
  rule('input_no_output', 280, (A, P, DW, C, F) => {
    ...
    return page({
      ...
      action: UL([
        '<b>이번 주</b> — 배운 것 하나를 <b>남에게 설명</b>하세요. 글이든 말이든 상관없습니다. 밖으로 나가면 됩니다.',
        '<b>이번 달</b> — 새 강의·새 자격을 하나 <b>줄이고</b>, 그 시간에 이미 배운 것으로 <b>한 건을 끝내세요.</b>',
        '<b>규칙</b> — 무엇을 배우든 <b>제출일을 먼저 정하고</b> 시작하세요. 마감이 없으면 이 사주는 끝내지 않습니다.'
      ]),
      ...
      blocks: [
        ...
        weakHap
          ...
          : P_(`이 구조는 힘이 약하지 않습니다. <b>큰 결정 한 번에 크게 흔들릴 수 있으니</b>
                금액이 커지는 자리에서는 반드시 문서와 제3자를 붙이세요.`)
      ],
  ...
  rule('no_gwan', 860, (A, P, DW, C, F) => {
    ...
    return page({
      ...
      action: UL([
        '<b>이번 주</b> — 지금 하는 일 하나에 <b>남이 아는 마감</b>을 붙이세요. 상대에게 날짜를 말하는 것만으로 됩니다.',
        '<b>이번 달</b> — 자율에 맡겨진 일과 마감이 있는 일 중, <b>마감 있는 쪽</b>에 시간을 더 배분하세요.',
        '<b>직업 선택</b> — 자율성이 큰 자리를 고를 때는 <b>스스로 마감 구조를 만들 수 있는지</b>를 먼저 확인하세요. 이 사주가 가장 자주 무너지는 지점입니다.'
      ]),
  ...
  rule('yongshin_timing', 180, (A, P, DW, C, F) => {
    ...
    return page({
      ...
      action: UL([
        `<b>지금</b> — ${EL_LIFE[A.yongshin].act}에 해당하는 일을 <b>한 가지</b> 골라 지금부터 쌓으세요.`,
        `<b>표시된 구간</b> — 그때가 되면 <b>규모를 키우세요.</b> 같은 노력에 결과가 더 붙는 때입니다.`,
  ...
  rule('friction_spots', 840, (A, P, DW, C, F) => {
    ...
    return page({
      ...
      blocks: [
        ...
        H(1, '이 표시를 쓰는 올바른 방법'),
        UL([
          '<b>피하려 하지 마세요.</b> 이 자리는 없어지지 않습니다. 대신 <b>미리 정해두면</b> 훨씬 작아집니다.',
          '<b>같은 문제가 3년, 5년 간격으로 돌아옵니다.</b> 그때마다 처음처럼 놀라지 않는 것만으로 절반은 해결됩니다.',
          '<b>기록해 두세요.</b> 언제 어떤 일로 부딪혔는지 적어두면 다음 번에 대응 시간이 확 줄어듭니다.'
        ]),
        BOX(`<b>사주의 이 부분은 검증이 쉽습니다.</b><br>
             위 표에 적힌 자리에서 <b>과거에 실제로 반복이 있었는지</b> 떠올려 보세요.
             맞으면 이 감명서의 나머지도 같은 기준으로 읽으시면 되고,
             전혀 아니라면 <b>태어난 시각</b>을 다시 확인해야 합니다. 시각이 바뀌면 이 표가 통째로 달라집니다.`, 'good')
      ],
      action: UL([
        '<b>이번 달</b> — 위 자리 중 하나를 골라, <b>지금 어긋나 있는 조건</b>(돈·시간·역할 중 하나)을 종이에 적으세요.',
        '<b>협상</b> — 상대에게 <b>한 문장</b>으로 말하세요. 참고 넘어가면 이 자리는 반드시 다시 옵니다.',
        '<b>기록</b> — 부딪힌 날짜와 내용을 남기세요. 이 사주에서 가장 값이 큰 습관입니다.'
      ]),
    ...
    return page({
      ...
      blocks: [
        ...
        BOX(`<b>그런데 이게 저절로 복이 되지는 않습니다.</b><br>
             ...
             <b>구체적으로 부탁하세요.</b> "도와주세요"가 아니라
             <b>"이 부분을 30분만 봐주실 수 있을까요"</b>처럼 범위와 시간을 정해 말하면
             이 사주는 거절당하는 일이 드뭅니다.`, 'good')
      ],
      action: UL([
        `<b>이번 달</b> — ${info.name} 쪽 사람 <b>한 명</b>에게, 범위를 정한 부탁을 하나 하세요.`,
        '<b>기준</b> — 부탁은 <b>①무엇을 ②얼마 동안 ③언제까지</b> 세 가지를 붙여서 하세요.',
        '<b>되돌려주기</b> — 받은 도움은 반드시 결과로 보고하세요. 이 사주의 인복은 <b>한 번이 아니라 반복</b>으로 옵니다.'
      ]),
  ...
  rule('gyeok', 200, (A, P, DW, C, F) => {
    ...
    return page({
      ...
      blocks: [
        ...
        BOX(`<b>이 면을 이렇게 쓰세요.</b><br>
             격은 등급이 아닙니다. <b>어떤 방식으로 살 때 이 사람이 가장 자기답게 되는가</b>를 가리킵니다.
             비어 있는 자리가 있다면, 그건 결핍이 아니라 <b>어디에 힘을 보태야 하는지</b>를 알려주는 지도입니다.`)
      ],
      action: UL([
        ok ? '<b>지금 방식을 유지하세요.</b> 이 사주는 중심이 서 있어, 방향을 자주 바꾸는 것이 오히려 손해입니다.'
          : '<b>비어 있는 자리를 사람과 절차로 메우세요.</b> 타고나지 않은 것은 빌려 쓰면 됩니다.',
        '<b>남과 비교하지 마세요.</b> 격은 서로 다른 방식이지 높고 낮음이 아닙니다.',
        '<b>운의 시기를 보세요.</b> 앞의 「언제가 좋은가」 면과 함께 읽어야 이 면이 완성됩니다.'
      ]),
    ...
    return page({
      ...
      blocks: ps.slice(0, 3).map((p, i) =>
        ...
        BOX(`<b>이 연결이 이 사주에서 가장 값진 부분입니다.</b><br>
             ...
             위에 적힌 연결이 실제로 쓰이는 자리를 직업과 생활에서 찾으세요.`, 'good')
      ]),
      action: UL([
        '<b>이번 달</b> — 위 연결에 해당하는 일을 <b>지금 하는 일 안에서</b> 찾아보세요. 대개 이미 하고 있습니다.',
    ...
    return page({
      ...
      blocks: [
        ...
        cold ? UL([
          ...
          '<b>겨울과 장마철</b>에 기운이 확연히 떨어집니다. 이 시기 일정을 미리 느슨하게 잡으세요.'
        ]) : hot ? UL([
        ...
        BOX(`<b>이 사주에 실제로 듣는 생활 규칙</b>
          <ul style="margin:2.5mm 0 0">
            ...
            ${hot ? '<li><b>일정 사이에 빈칸을 먼저 넣으세요.</b> 다 채운 일정표가 이 사주에는 가장 위험합니다.</li><li><b>수분과 수면을 숫자로 관리하세요.</b></li>' : ''}
            ${인 >= 3 && 식 === 0 ? '<li><b>몸을 쓰는 단순 운동.</b> 머리를 비우는 것이 목적입니다.</li><li><b>밤에 내린 결론은 다음 날 낮에 한 번 더 보세요.</b> 이 사주는 밤 판단이 어둡습니다.</li>' : ''}
          </ul>`),
        P_(`<span class="rd-dim">※ 증상이 계속되거나 심해지면 사주가 아니라 <b>의료진의 진료와 검사</b>가 먼저입니다.
            이 면은 병을 예측하지 않으며 생활 조건만 다룹니다.</span>`)
      ],
      action: UL([
        '<b>이번 주</b> — 위 규칙 중 <b>하나만</b> 고르세요. 여러 개를 동시에 시작하면 이 사주는 다 놓칩니다.',
        '<b>한 달 뒤</b> — 그 하나를 실제로 지켰는지 확인하세요. 지켰다면 그때 두 번째를 더하세요.',
  ...
  const CONF_LABEL = { firm: n => '신호 ' + n + '개', cond: () => '신호 1개', check: () => '조용' };
    ...
    return page({
      ...
      blocks: [
        ...
        rows.map(r => {
          ...
          let detail = r.sig.fb === '반음'
            ...
            : r.sig.yong
              ...
              : `크게 흔들리지 않습니다. 하던 것을 이어가세요`;
          // 신뢰도 계층 — 신호가 하나면 조건을 조언에 녹이고, 없으면 확인 방법을 준다.
          if (tier === 'cond') detail += ` — 연초에 실제 조짐이 확인되면 그때 크게 가도 늦지 않습니다`;
          else if (tier === 'check') detail = `밀지도 막지도 않는 조용한 해입니다. 연초 상황을 확인하고 계획의 크기를 정하세요`;
        ...
        BOX(`<b>이 표를 읽는 법</b><br>
          <b>밀 때</b> — 계약·이직·시작처럼 <b>내가 정해서 벌이는 일</b>을 여기 배치하세요.<br>
          <b>조정할 때</b> — 새로 벌이기보다 <b>이미 있는 것의 조건을 고치는</b> 데 씁니다.<br>
          <b>지킬 때</b> — 무슨 일이 생기는 해가 아니라, <b>큰 결정을 미루면 손해가 적은</b> 해입니다.<br><br>
          <b>신호 개수</b> — 같은 판단이라도 <b>서로 다른 계산 몇 개가 같은 방향을 가리켰는가</b>입니다.
          신호가 2개 이상인 해는 그대로 믿으셔도 되고, 1개인 해는 연초 상황을 보고 정하시고,
          「조용」인 해는 표보다 <b>그해의 실제 상황</b>을 믿으세요.`),
        first ? P_(confSpan(confTier(first.sig.ev), first.sig.ev,
          confTier(first.sig.ev) === 'firm'
            ? `가장 가까운 <b>밀 때</b>는 <b>${first.s.year}년(만 ${first.age}세)</b>입니다.
               미뤄둔 큰 결정이 있다면 이 해를 먼저 고려하세요.`
            : `가장 가까운 <b>밀 때</b> 후보는 <b>${first.s.year}년(만 ${first.age}세)</b>입니다.
               그 해 초에 실제 제안이나 기회가 확인되면 그때 무게를 실으세요. 그 전까지는 준비가 이 표의 결론입니다.`)) : '',
  ...
  rule('prose_decision', 993, function (A, P, DW, C, F) {
    ...
    const verdict = best && best.sig.kind === 'push'
      ? confSpan(bestTier, bestEv, bestTier === 'firm'
        ? '물으신 시한 안에서 고르라면 <b>' + best.s.year + '년</b>입니다. ' + (best.grpHit || best.sig.yong ? '질문하신 쪽의 기운이 실제로 들어오고 마찰도 적은 해라서입니다.' : '마찰이 가장 적은 해라서입니다.') +
          ' 여러 신호가 같은 해를 가리키고 있으니 이 해를 중심에 놓고 준비하세요. 다만 해가 좋다는 것은 조건이 좋다는 뜻이 아닙니다 — <b>그 해에 실제 조건(금액·상대·계약)이 갖춰졌을 때</b> 움직이는 겁니다.'
        : '물으신 시한 안에서 고르라면 <b>' + best.s.year + '년</b>입니다. 다만 이 해는 문이 반쯤 열린 해라, 그 해 초에 물으신 쪽의 움직임 — 제안·자리·상대 — 이 실제로 확인되면 그때 무게를 실으시고, 조용하면 해를 넘겨도 됩니다. <b>실제 조건(금액·상대·계약)이 갖춰졌을 때</b> 움직이는 원칙은 같습니다.')
      : best
        ...
        blocks: [
          ...
          best
            ? BOX(`<b>이 표에서 딱 하나만 기억하신다면</b><br>
                ...
                ${best.now ? '지금이 바로 그 구간입니다. 미루지 마세요.'
      ...
      return page({
        ...
        blocks: [
          ...
          hot.length ? BOX(`<b>이 해에 특히 조심할 달</b><br>
            ...
            새로 벌이기보다 <b>계약·일정·사람 관계를 다시 확인</b>하는 데 쓰세요.
        ...
        `<div class="yby">` + rows.map(r => `
          <div class="yby-row${r.sw.yongshinHit ? ' yong' : ''}${r.clash.length ? ' clash' : ''}">
            ...
            <div class="yby-b">
              ...
              ${r.clash.length ? `<p class="yby-warn">⚠ ${r.clash.map(h => (POS_LIFE[h.k] || {}).name).join('·')} 쪽 조건이 흔들립니다. 이 해에는 계약·이사·이직처럼 되돌리기 어려운 결정을 한 번 더 확인하세요.</p>` : ''}
              ${r.hap.length ? `<p class="yby-good">○ ${r.hap.map(h => (POS_LIFE[h.k] || {}).name).join('·')} 쪽에서 이야기가 구체화됩니다. 미뤄둔 대화를 이 해에 하세요.</p>` : ''}
            </div>
          </div>`).join('') + `</div>`
      ],
      action: UL([
    ...
    rule('prose_year_' + off, 958 - off, function (A, P, DW, C, F) {
      ...
      const q4 = sw.yongshinHit
        ? '그리고 이 해에는 <b>' + C.you + '에게 모자란 ' + EL_KO[A.yongshin] + '의 기운이 함께 들어옵니다.</b> ' +
          ...
          '준비해 둔 것이 있으면 이 해에 꺼내세요. 없으면 이 해는 그냥 지나갑니다.'
      ...
      const q6 = tier === 'firm'
        ? confSpan('firm', sig.ev, '이 해의 흐름은 여러 신호가 같은 방향을 가리키고 있어서, 위에 적은 대로 믿고 계획을 세우셔도 됩니다.')
        : tier === 'cond'
        ? confSpan('cond', sig.ev, '다만 이 해는 흐름이 반쯤만 열려 있습니다. 그래서 연초가 중요합니다 — 위에 적은 방향의 움직임(제안, 자리, 돈 이야기)이 실제로 확인되면 그때 크게 움직이시고, 조용하면 이 해는 준비에 쓰세요. 준비한 사람에게는 다음 해가 두 배로 열립니다.')
        : confSpan('check', sig.ev, '그리고 이 해는 사주가 특별히 밀지도, 막지도 않는 조용한 해입니다. 이런 해를 아는 것도 복입니다 — 남들이 흔들릴 때 ' + C.you + '은 쌓을 수 있으니까요. 연초에 딱 세 가지만 확인해 보세요. 새 제안이 오는지, 소속이나 역할이 바뀌는지, 큰돈 나갈 일이 생기는지. 하나라도 나타나면 그쪽 장을 다시 펴면 됩니다.');
  ...
  // 궁위별 합충 서술 — 업계 말(원국·충·합·간지 이름) 없이 그 시기의 생활어로만 적는다.
  // (8/12 14차: 사장님 "뭔 말인지 모르겠어" — 기호를 뜻으로 번역하는 게 이 층의 일이다. 기호는 근거줄로.)
  const TOUCH_SENT = {
    충: {
      년: ' 이 출발점에는 큰 변화의 표시가 함께 적혀 있습니다. 어린 시절이나 집안에 이사·형편 변화·가족 구성의 변화처럼 판이 한 번 새로 짜이는 일이 있는 그림입니다. 무너진 것이 아니라 다시 지은 것이니, 그 변화를 흠으로 여기지 않으셔도 됩니다.',
      월: ' 이 시기에는 자리를 한 번 크게 옮기는 표시가 함께 적혀 있습니다. 직장·업종·사는 지역 같은 것이 통째로 바뀌는 종류의 변화입니다. 미리 알고 준비하면 이 변화는 사고가 아니라 기회가 됩니다.',
      일: ' 삶의 한가운데, 가장 가까운 관계의 자리에 큰 조정의 표시가 있습니다. 배우자·단짝과 틀어진다는 뜻이 아니라, 사는 방식과 역할 분담을 중간에 한 번 다시 정하게 된다는 뜻입니다. 그 대화를 피하지 않는 것이 이 자리의 숙제입니다.',
      시: ' 말년의 자리에 큰 변화의 표시가 있습니다. 나이 들어 사는 곳이나 맡는 역할이 한 번 크게 바뀌는 그림이니, 늦은 변화를 두려워하기보다 미리 그려 두면 그 변화가 말년을 넓혀 줍니다.'
    },
    합: {
      년: ' 그리고 이 시기에 맺어진 인연 — 가족, 어릴 적 사람들 — 은 유난히 오래갑니다. 나이 들어서도 곁에 남는 사람들입니다.',
      월: ' 그리고 이 시기의 인연과 자리는 한 번 맺으면 오래갑니다. 쉽게 끊어지지 않는 대신 잘못 맺어도 오래가니, 처음 고를 때 신중해야 하는 자리입니다.',
      일: ' 그리고 내 곁의 인연은 한 번 맺으면 질기게 이어집니다. 오래가는 것이 이 자리의 복이고, 그래서 처음의 선택이 더 무겁습니다.',
      시: ' 그리고 말년에 맺는 인연과 맡는 자리는 오래갑니다. 늦게 만난 사람, 늦게 시작한 일이 끝까지 함께 가는 그림입니다.'
    },
    마찰: {
      년: ' 한 가지 덧붙이면, 집안·가족 일은 남들보다 자주 신경 쓸 일이 생기는 자리입니다. 큰일이 난다는 뜻이 아니라, 가족 사이의 약속·돈·역할 문제를 미루지 말고 그때그때 정리하는 편이 좋다는 뜻입니다.',
      월: ' 한 가지 덧붙이면, 일터에서는 조건을 자주 다시 맞추게 됩니다. 계약·업무 범위·처우 같은 것을 애매하게 두면 꼭 탈이 나니, 문서로 분명히 해 두는 습관이 이 자리를 지켜 줍니다.',
      일: ' 한 가지 덧붙이면, 가까운 사이일수록 사소한 것을 애매하게 두지 않는 편이 좋습니다. 돈·집안일·약속을 분명히 하는 것이 정 없는 게 아니라, 이 관계를 오래가게 하는 방법입니다.',
      시: ' 한 가지 덧붙이면, 말년의 재산·역할 문제는 미리미리 정리해 두는 편이 좋은 자리입니다. 애매하게 남겨 두면 그것이 나중에 가까운 사람들의 숙제가 됩니다.'
    }
  };
  function seasonTouch(ps, pos) {
    let out = '';
    if (ps.touch.some(function (t) { return t.kind === '충'; })) out += TOUCH_SENT.충[pos] || '';
    else if (ps.touch.some(function (t) { return t.kind === '합'; })) out += TOUCH_SENT.합[pos] || '';
    else if (ps.touch.some(function (t) { return t.kind === '마찰'; })) out += TOUCH_SENT.마찰[pos] || '';
    if (ps.gm) out += ' 그리고 이 자리에는 옛말로 「빈 곳」이라 부르는 표시가 있습니다. 이 시기의 소유와 직함에 힘을 주기보다, 몸에 남는 기술과 역할을 쌓는 편이 결국 남습니다.';
    return out;
  }
  // 근거줄용 — 어떤 기둥에 어떤 합충이 걸렸는지 기호 그대로
  function touchEvidence(ps) {
    return ps.touch.length ? ps.touch.map(function (t) { return t.k + '(' + t.pair + ')'; }).join('·') : '없음';
  }
  ...
  rule('prose_seasons', 992, function (A, P, DW, C, F) {
    ...
    ['년', '월', '일', '시'].forEach(function (pos) {
      ...
      // 겉결과 속결이 다를 때 — 시기마다 시제와 장면을 다르게 말한다 (8/12: "말년인데 과거형" 사고 수정)
      const GRP_SHORT = { 비겁: '내 힘으로 서려는', 식상: '만들고 표현하는', 재성: '실속을 챙기는', 관성: '책임을 지는', 인성: '배우고 기대는' };
      const MISMATCH = {
        년: ' 겉에서 본 집안 분위기는 ' + GRP_SHORT[ps.grpS || ps.grp] + ' 쪽, 안살림의 실제 결은 ' + GRP_SHORT[ps.grp] + ' 쪽이어서 — 밖에서 보던 집과 안에서 살던 집이 똑같지는 않았을 것입니다.',
        월: ' 명함에 적히는 역할은 ' + GRP_SHORT[ps.grpS || ps.grp] + ' 쪽인데 몸이 실제로 하는 일은 ' + GRP_SHORT[ps.grp] + ' 쪽으로 흐릅니다. 직함과 실제 일이 조금 다르게 굴러간다는 뜻인데, 둘 중 하나를 버릴 필요 없이 둘 다 내 것으로 쓰면 됩니다.',
        시: ' 말년의 겉모양은 ' + GRP_SHORT[ps.grpS || ps.grp] + ' 쪽이고 속에서 힘을 내는 것은 ' + GRP_SHORT[ps.grp] + ' 쪽이 됩니다. 두 흐름을 다 쓰는 사람이 말년이 넓어집니다.'
      };
    ...
    return page({
      ...
      evidence: ['년', '월', '일', '시'].map(function (pos) {
        const pil = { 년: P.year, 월: P.month, 일: P.day, 시: P.hour }[pos];
        if (!pil) return pos + '주 미상(시각 미입력)';
        const st = pillarStory(A, P, C, pos);
        return pos + '주 ' + st.ganji + '(' + (st.gs ? st.gs + '/' : '') + st.gb + ') 합충 ' + touchEvidence(st);
  ...
  rule('prose_people', 991.5, function (A, P, DW, C, F) {
    ...
    const p1 = '<b>부모와 집안(년주 ' + yr.ganji + ')</b> — 년주에 앉은 기운은 <b>' + (yr.gs || yr.gb) + '</b>입니다. ' +
      ...
      ' 집안이 내게 준 것과 내가 집안에 갚는 것은 결국 이 기운의 언어로 오갑니다.' + seasonTouch(yr, '년');

    const PEOPLE_SOCIAL = {
      비겁: '동료이자 경쟁자인 사람들입니다. 같은 편일 때 가장 든든하고, 몫을 나눌 때 가장 위험하니 성과의 이름표를 늘 분명히 해 두세요.',
    ...
    const p4 = hr
      ? '<b>자식과 아랫사람(시주 ' + hr.ganji + ')</b> — 시주의 기운은 <b>' + (hr.gs || hr.gb) + '</b>입니다. ' + (GONGWI_APPLY['시'][hr.grpS || hr.grp] || '') +
        ' 자식이 있든 없든 이 자리는 작동합니다 — 후배·제자·내가 만든 것들이 전부 이 자리의 식구입니다.' + seasonTouch(hr, '시')
      : '<b>자식과 아랫사람(시주)</b> — 태어난 시각이 확인되지 않아 이 자리는 정직하게 비워 둡니다. 시각을 확인해 다시 뽑으면 이 문단이 채워집니다.';

    const p5 = nobles.length
      ? '마지막으로, 이 원국에는 <b>도와주는 사람의 표지(' + [...new Set(nobles.map(function (x) { return x.name; }))].join('·') + ')</b>가 ' + nobles.length + '곳에 잡힙니다. 결정적인 순간에 손을 내미는 사람이 반복해서 나타나는 그림이니, 도움을 받으면 반드시 갚아 두세요. 이 표지는 갚는 사람에게만 계속 작동합니다.'
  ...
  rule('prose_work', 991, function (A, P, DW, C, F) {
    ...
    const p1 = gwan >= 2
      ...
      : '일의 뼈대부터 봅니다. 이 사주에는 <b>자리와 책임의 기운(관성)이 없습니다.</b> 조직의 사다리로 크는 그림이 아니라 <b>내 이름과 내 실력으로 서는 그림</b>이라는 뜻입니다. 직장에 있더라도 직함이 아니라 "그 일은 그 사람"이라는 평판을 쌓는 쪽이 이 구조의 정답입니다.';
    ...
    const p3 = pats.length
      ? '그리고 이 사주에는 일에 관한 한 <b>' + pats.map(function (pt) { return (PATTERN_DOMAIN[pt.id] || {}).easy || pt.title; }).join('과 ') + '</b>의 연결이 이미 놓여 있습니다. ' + pats.map(function (pt) { return pt.reading; }).join(' ') + ' 진로를 고를 일이 생기면 이 연결이 작동하는 쪽을 고르세요 — 같은 노력이 두 배로 계산되는 자리입니다.'
      : '';
